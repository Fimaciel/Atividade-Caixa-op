...
Select * FROM Venda as v inner join cliente as c ON v.id_cliente_fk = c.id;
Select * FROM Venda as v left join cliente as c ON v.id_cliente_fk = c.id;
Select * FROM Venda as v right join cliente as c ON v.id_cliente_fk = c.id;


Select valor_total, nome FROM Venda as v inner join cliente as c ON v.id_cliente_fk = c.id;
Select desconto, valor_total, nome FROM Venda as v left join cliente as c ON v.id_cliente_fk = c.id;
Select  nome, cpf FROM Venda as v right join cliente as c ON v.id_cliente_fk = c.id;
...
select * from vendaServico where valor_unitario > (select avg(valor_unitario) from vendaServico );


select * from recebimentos as r
 inner join caixa as c on r.caixa_fk = c.id
...
SELECT * FROM servicos;
 ...
 SELECT vs.id, s.descricao, vs.valor_unitario, s.valor FROM vendaServico as vs
 inner join servicos as s on vs.servico_fk = s.id;
 
 select v.id, v.valor_total, c.nome from venda as v 
 left join cliente as c on v.id_cliente_fk = c.id;
 
 Select * from despesas as d inner join fornecedores as f on d.fornecedor_fk = f.id;
